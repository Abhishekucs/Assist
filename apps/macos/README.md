...
Assist is a native macOS utility for voice annotation, screenshots, and local clipboard history.
...
Claude Code and Codex integrations have been retired. Assist now focuses on
voice annotation, screenshots, and clipboard history.

On normal launch, before license activation, Assist removes its own old handlers
from the default provider configuration files and any custom directories known
through the previous preference, `CODEX_HOME`, or `CLAUDE_CONFIG_DIR`. Unrelated
hooks and settings are preserved. Successfully checked paths are recorded; failed
paths are logged and retried on a later launch. Obsolete preferences are removed
only after cleanup succeeds. Stale, user-owned integration sockets are removed;
active sockets and unrelated files are left alone.
