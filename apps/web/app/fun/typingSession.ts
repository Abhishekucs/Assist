...
export function createTypingSession(seed = 0): TypingSession {
  return { words: makePassage(seed), value: "", phase: "ready", elapsedMs: 0,
...
function advanceClock(state: TypingSession, now: number): TypingSession {
  ...
  const elapsedMs = Math.min(testDuration * 1000, state.elapsedMs + Math.max(0, now - state.updatedAt));
  const complete = elapsedMs >= testDuration * 1000;
...
export function typingReducer(state: TypingSession, action: TypingAction): TypingSession {
  if (action.type === "restart") return createTypingSession(action.seed);
...
export function typingMetrics(state: TypingSession) {
  ...
  return {
    ...
    remaining: Math.max(0, Math.ceil(testDuration - state.elapsedMs / 1000))
  };
}
